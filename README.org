...
Obtain the replication package for Bauer and Rudebusch (2023) from [[https://www.michaeldbauer.com/][Michael Bauer's website]].
Estimate the parameter values of their interest rate model (~estimate_uc.r~)
and save the results at ~./data_processed/uc_estimates_y10.RData~.

Then run the model to generate future projections of interest rate:
...
I compare the projections of interest rate and per-capita income growth
and choose preference parameters (rho and eta)
that makes the distributions of the two projections as close as possible.
