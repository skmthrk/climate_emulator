...
We address this through a two-step approach.
First, we employ a linear model and calibrate the model parameters based on the impulse response experiment of Joos et al. (2013).
Then, taking the calibrated carbon cycle parameters given,
we calibrate the scaling factor as a function of cumulative carbon emission to capture the non-linear feedback effects.
...
With the carbon cycle matrix A calibrated as described,
we incorporate a non-linear scaling factor, exp(gamma0 + gamma1 * M) * A, where M represents cumulative carbon emissions.
The parameter gamma1 is intended to be negative, reflecting the potential weakening of the carbon cycle as the total carbon in the system increases.
